    ...
    <div class="bg-white p-2 border-solid border border-blueGray-light mb-5">
        @if($profile->agency)
            <div class="w-full p-1 md-p-4 mb-4 bg-white flex items-center justify-between">
                ...
                <a href="{{route('profile.main', $profile->agency['agent']['username'] )}}"><button class="w-36 md:w-52 px-1 md:px-10 font-bold py-5 md:py-3 md:text-sm border-none bg-curious-blue text-white cursor-pointer">
                    عرض الملف الشخصي
                </button></a>
            </div>
        @else
