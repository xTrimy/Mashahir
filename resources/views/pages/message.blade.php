...
@section('title', "التواصل مع " . $celebrity->name)
...
@section('contents')
    <div class="w-full py-12 px-8 bg-white">
        <div class="flex items-center mb-8">
            ...
            <h1 class="text-3xl mr-8">طلب تواصل مع {{$celebrity->name}}</h1>
        </div>
        <div class="w-full">
            <form method="POST" action="/messages/create/{{$celebrity->username}}" class="w-full">
                @csrf
                <label>
                    <p class="text-lg mb-2">عنوان الطلب</p>
                    <input type="text" name="subject" placeholder="أدخل عنوان الرسالة" class="w-full border-2 border-blue-200 bg-blue-50 outline-none focus:ring-1 ring-curious-blue py-2 px-4">
                </label>
                <label>
                    <p class="text-lg mb-2 mt-4">نص الرسالة</p>
                    <textarea name="message" placeholder="أدخل نص الرسالة" class="w-full border-2 border-blue-200 bg-blue-50 outline-none focus:ring-1 ring-curious-blue py-2 px-4" name="" id="" cols="30" rows="10"></textarea>
                </label>
                <div class="flex">
